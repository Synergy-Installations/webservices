...
function storageUrl(storagePath: string): string {
    return `https://${STORAGE_HOSTNAME}/${encodeURIComponent(BUNNY_STORAGE_ZONE)}/${encodePathSegments(storagePath)
        }`;
}
...
function sizeLimitStream(maxBytes: number): TransformStream<Uint8Array, Uint8Array> {
    let total = 0;
    return new TransformStream({
        transform(chunk, controller) {
            ...
            if (total > maxBytes) {
                controller.error(
                    new Error(`payload exceeds ${maxBytes} bytes`),
                );
                return;
            }
...
async function handle(request: Request): Promise<Response> {
    ...
    if (configErrors.length > 0) {
        return json(500, { error: "missing_config", missing: configErrors }, origin);
    }
