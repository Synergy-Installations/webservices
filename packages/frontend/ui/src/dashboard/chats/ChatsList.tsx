"use client";
import { ChatInterface } from "@com.synergy/frontend-backend-dashboard/chat";
import {
  useAddChatMutation,
  useGetChatsQuery,
...
export const ChatsList = (props: StepsListProps) => {
  const {
    params: { id: submitId },
    className,
  } = props;
  ...
  const [addChat, { isLoading: isAddStepLoading, error: addStepError }] =
    useAddChatMutation();

  const [newChat, setNewChat] = useState<Partial<ChatInterface>>({
    order: chats?.data?.chats.length ? chats.data.chats.length + 1 : 1,
    title: "",
    description: "",
  ...
  useEffect(() => {
    if (chats?.data?.chats.length) {
      setNewChat((prev) => ({
        ...prev,
        order: chats.data.chats.length + 1,
      }));
    }
  ...
  const createNewChat = () => {
    addChat({
      submitId: newChat.submitId || new Types.ObjectId(submitId),
      ...newChat,
    });
    setNewChat({
      ...newChat,
  ...
  return (
    ...
    >
      {isGetStepsLoading ? (
        ...
        chats?.data?.chats.map((chat: any, index: number) => (
          <Link
            href={`/dashboard/submits/${submitId}/chats/${chat._id}/messages`}
            key={chat._id}
      ...
      <form className="flex justify-between items-center gap-2 group border-2 border-dashed border-synergy-light-grey p-2 rounded-xl hover:bg-synergy-light-grey transition-colors duration-200">
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                e.preventDefault();
                createNewChat();
                console.log("Step submitted:", newChat);
              }
            }}
          />
          {/* <div className="flex items-center gap-2 mt-2">
        ...
        <button
          ...
          disabled={isAddStepLoading || newChat.title === ""}
        >
