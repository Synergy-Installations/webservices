...
import { useAddAssetMutation } from "@com.synergy/frontend-backend-dashboard/assetApi";
import { Types } from "mongoose";

/* eslint-disable-next-line */
export interface FileUploadVaultProps {
...
export const FileUploadVault = (props: FileUploadVaultProps) => {
  const {
    ...
    acceptedFiles.forEach((file) => {
      ...
      reader.onload = async () => {
        ...
        if (response.ok) {
          ...
          setAssets((prev) =>
            prev.map((file) =>
              file.filePath === fileName ? { ...file, status: "error" } : file
            )
          );
        ...
        {assets?.map((file: any, index: number) => (
          ...
          >
            ...
            {/* <button
              ...
              onClick={() => {
                setAssets((prev) => 
                  prev.filter((fileFilter) => fileFilter.filePath !== file.filePath)
                );
              }}
            >
              <svg
...
export default FileUploadVault;
