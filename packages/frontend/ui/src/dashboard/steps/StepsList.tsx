"use client";
import { StepInterface } from "@com.synergy/frontend-backend-dashboard/step";
import {
  useAddStepMutation,
  useGetStepsQuery,
...
export const StepsList = (props: StepsListProps) => {
  ...
  const {
    ...
    error,
  } = useGetStepsQuery(submitId, {
    refetchOnFocus: true,
    refetchOnReconnect: true,
    refetchOnMountOrArgChange: true,
  });

  const [addStep, { isLoading: isAddStepLoading, error: addStepError }] =
    useAddStepMutation();
  ...
  useEffect(() => {
    if (submits?.data?.steps.length) {
      setNewStep((prev) => ({
        ...prev,
        order: submits.data.steps.length + 1,
      }));
    }
  }, [submits?.data?.steps.length]);

  const createNewStep = () => {
    addStep({
      submitId: newStep.submitId || new Types.ObjectId(submitId),
      ...newStep,
    });
    setNewStep({
      ...newStep,
  ...
  return (
    ...
    >
      {isGetStepsLoading ? (
        ...
        submits?.data?.steps.map((step: any, index: number) => (
          <Link
            href={`/dashboard/submits/${submitId}/steps/${step._id}/general`}
            key={step._id}
      ...
      <form className="flex justify-between items-center gap-2 group border-2 border-dashed border-synergy-light-grey p-2 rounded-xl hover:bg-synergy-light-grey transition-colors duration-200">
        <div className="w-full">
          ...
          <div className="flex items-center gap-2 mt-2">
            <div className="w-max">
              <div
                className="w-4 h-4 rounded-full"
                style={{ backgroundColor: newStep.status?.color }}
              ></div>
            </div>
        ...
        <button
          ...
          disabled={isAddStepLoading || newStep.title === ""}
        >
