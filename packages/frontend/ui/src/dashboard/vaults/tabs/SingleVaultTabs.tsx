...
import { IsUpdateVaultLoadingState } from "@com.synergy/frontend-ui/VaultSubmitSettings";

/* eslint-disable-next-line */
export interface SingleVaultTabsProps {
  params: { id: string; vaultId?: string };
  SingleVaultTabsEdit?: SingleVaultTabsEdit;
}

interface SingleVaultTabsEdit {
...
export const SingleVaultTabs = (props: SingleVaultTabsProps) => {
  const {
    params: { id, vaultId },
    SingleVaultTabsEdit,
  } = props;
  ...
  const isVault = path.includes("assets");
  ...
  return (
    <div className="">
      <ul className="grid grid-cols-3 xs:flex text-sm font-medium text-gray-500 dark:text-gray-400 !p-2 gap-2 overflow-x-auto">
        ...
        <li>
          <Link
            href={`/dashboard/submits/${id}/vaults/${vaultId}/assets`}
            className={`inline-flex items-center justify-center px-4 py-2 ${isVault ? "text-white bg-blue-700 dark:bg-blue-600" : "hover:text-gray-900 bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 dark:hover:text-white"} rounded-lg active w-full `}
          >
            <svg
              className={`w-4 h-4 me-2 ${isVault ? "text-white" : "text-gray-500 dark:text-gray-400"}`}
              aria-hidden="true"
              xmlns="http://www.w3.org/2000/svg"
              fill="currentColor"
              viewBox="0 0 20 20"
        ...
        <li>
          <Link
            href={`/dashboard/submits/${id}/vaults/${vaultId}/settings`}
        ...
        {SingleVaultTabsEdit && (
          <li className="">
            <button
              onClick={() =>
                SingleVaultTabsEdit.setEditVaultToggle(
                  !SingleVaultTabsEdit.editVaultToggle
                )
              }
            ...
            >
              <span className="">
                {SingleVaultTabsEdit.editVaultToggle ? "View" : "Edit"}
              </span>
            </button>
          </li>
        )}
        {SingleVaultTabsEdit &&
          SingleVaultTabsEdit.editVaultToggle &&
          // Check if the saveVaultToggle is defined
          SingleVaultTabsEdit.saveVaultToggle !== undefined && (
            <li className="">
              <button
                onClick={() =>
                  SingleVaultTabsEdit?.setSaveVaultToggle &&
                  SingleVaultTabsEdit.setSaveVaultToggle(true)
                }
              ...
              >
                ...
                <div className="">
                  {(SingleVaultTabsEdit?.isUpdateVaultLoading?.titleDesc ||
                    SingleVaultTabsEdit?.isUpdateVaultLoading
                      ?.membersRights) && (
                    <svg
                      aria-hidden="true"
                      role="status"
...
export default SingleVaultTabs;
