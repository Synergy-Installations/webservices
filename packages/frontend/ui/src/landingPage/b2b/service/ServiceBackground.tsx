...
export interface ServiceBackgroundProps {
  translationService: string;
  translationSubService: string;
}

export const ServiceBackground = (props: ServiceBackgroundProps) => {
  const { translationService, translationSubService } = props;
  const t = useTranslations(
    `LandingPage.B2B.Services.${translationService}.SubServices.${translationSubService}.Background`
  );
  ...
  const boxesLeftKeys = Object.keys(
    ...
    ].Background.boxes.boxLeft.list
  );
  const boxesRightKeys = Object.keys(
    messages.LandingPage.B2B.Services[translationService].SubServices[
      translationSubService
    ].Background.boxes.boxRight.list
  );
