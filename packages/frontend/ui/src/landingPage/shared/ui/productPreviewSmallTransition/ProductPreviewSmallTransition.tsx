    ...
    <>
      {props.products.map((product, index) => (
        <Transition show={tab === index + 1} key={index}>
          ...
          {/* <div
            key={index}
            className="transition ease-in-out data-[closed]:opacity-0 data-[enter]:duration-700 data-[enter]:data-[closed]:-translate-y-8 data-[closed]:absolute data-[leave]:duration-300 data-[leave]:data-[closed]:translate-y-8"
          >
            <ProductPreviewSmall
              image={{ src: product.image.src, alt: product.image.alt }}
              name={product.title}
